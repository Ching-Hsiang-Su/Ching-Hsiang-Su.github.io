#!/usr/bin/env python3
"""Resize portfolio photos and export them as numbered WebP files."""
...
def convert_image(
    ...
    intermediate = temporary_directory / f"{destination.stem}-{maximum_width}.png"
    run([
        "sips",
        "-Z",
        str(maximum_width),
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--thumb-width", type=int, default=720, help="縮圖最長邊上限（預設 720）")
...
def main() -> None:
    ...
    if args.start is not None and args.start < 1:
        fail("--start 必須大於 0")
    if args.max_width < 1 or args.thumb_width < 1:
        fail("圖片尺寸必須大於 0")
    if not 1 <= args.quality <= 100 or not 1 <= args.thumbnail_quality <= 100:
    ...
    print("縮圖：" + (f"最長邊 {args.thumb_width}px，WebP 品質 {args.thumbnail_quality}" if make_thumbnails else "不建立"))

    if args.dry_run:
        for source, (full, thumbnail) in zip(sources, outputs):
    ...
    with tempfile.TemporaryDirectory(prefix="portfolio-images-") as temporary:
        ...
        for source, (full, thumbnail) in zip(sources, outputs):
            ...
            convert_image(source, full, args.max_width, args.quality, temporary_directory)
            if thumbnail is not None:
                convert_image(
                    source,
                    thumbnail,
